...
class MainWindow(QMainWindow):
    ...
    MAX_RECENT_FILES = 10
    STANDARD_VIEW_LABELS = {'front': 'Front', 'back': 'Back', 'left': 'Left', 'right': 'Right', 'top': 'Top', 'bottom': 'Bottom', 'isometric': 'Isometric'}
    INSPECT_ACTIONS = {'select': 'Select', 'distance': 'Distance', 'angle': 'Angle', 'face_area': 'Face Area'}
    PICK_PREFERENCES = {'balanced': 'Balanced', 'prefer_point': 'Prefer Point', 'prefer_face': 'Prefer Face'}
    ...
    def _load_recent_files(self):
        ...
        return [path for path in (recent_files or []) if isinstance(path, str) and path]
    ...
    def _remove_recent_file(self, file_path):
        normalized_path = os.path.normpath(file_path)
        before = len(self.recent_files)
        self.recent_files = [path for path in self.recent_files if os.path.normpath(path) != normalized_path]
        if len(self.recent_files) != before:
            self._save_settings()
            self._update_recent_files_menu()
    ...
    def open_model_file(self, file_path, from_recent=False):
        ...
        if success:
            ...
            self.setWindowTitle(f"PyQtGLMeshViewer — {os.path.basename(file_path)}")
            self.show_status_message(f"Loaded: {file_path}", 4000)
        elif from_recent:
            self._remove_recent_file(file_path)
        return success
    ...
    def set_standard_view(self, view_name):
        ...
        self.show_status_message(f"View: {self.STANDARD_VIEW_LABELS[view_name]}", 2000)
    ...
    def set_pick_preference(self, preference):
        ...
        self.pick_status_label.setText(f"Pick: {self.PICK_PREFERENCES[preference]}")
    ...
    def export_screenshot(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Export Screenshot", f"viewer_screenshot_{timestamp}.png", "PNG Image (*.png)")
        if not file_path:
            return False
        if not file_path.lower().endswith('.png'):
            file_path += '.png'
        if self.gl_widget.capture_viewport(file_path):
            self.show_status_message(f"Screenshot saved: {file_path}", 4000)
            return True
    ...
    def export_inspection_report(self):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        base_path, _ = QFileDialog.getSaveFileName(self, "Export Inspection Report", f"inspection_report_{timestamp}", "Inspection Report (*.json *.png);;All Files (*)")
        if not base_path:
            return False
        root, _ = os.path.splitext(base_path)
        result = self.gl_widget.export_inspection_report(root or base_path)
        if result:
            self.show_status_message(f"Inspection report saved: {result['json_path']}", 5000)
            return True
    ...
    def _sync_inspect_actions(self, snapshot):
        ...
        if pick_preference in self.pick_preference_actions:
            ...
            self.pick_status_label.setText(f"Pick: {self.PICK_PREFERENCES[pick_preference]}")
